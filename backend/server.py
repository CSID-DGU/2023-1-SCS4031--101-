...
def aow_response_video():
    ...
    s1 = float(request.args.get('s1', 1.0)) * 0.1
    s2 = float(request.args.get('s2', 1.0)) * 0.1
...
def aou_response_video():
    ...
    # r3 = float(request.args.get('r3', 304))
    # r4 = float(request.args.get('r4', 359))
    r1 = float(request.args.get('r1', 0))
    r2 = float(request.args.get('r2', 40))
    r3 = float(request.args.get('r3', 145))
    r4 = float(request.args.get('r4', 179))
    # c1 = float(request.args.get('c1', 181))
    # c2 = float(request.args.get('c2', 231))
    c1 = float(request.args.get('c1', 90))
    c2 = float(request.args.get('c2', 115))
    s1 = float(request.args.get('s1', 1.0)) * 1.5 # * 10
    s2 = float(request.args.get('s2', 1.0)) * 1.5 # * 10
    print(f'r1: {r1}, r2: {r2}, r3: {r3}, r4: {r4}, c1: {c1}, c2: {c2}, s1: {s1}, s2: {s2}')
...
def aou_color_filter(img, r1, r2, r3, r4, c1, c2, s1, s2):
    ...
    lower_red1 = np.array([r1, 0, 0])
    upper_red1 = np.array([r2, 255, 255])
    mask_red1 = cv2.inRange(hsv, lower_red1, upper_red1)
    hsv[mask_red1 == 255, 1] = np.clip(hsv[mask_red1 == 255, 1] * s1, 0, 100)

    lower_red2 = np.array([r3, 0, 0])
    upper_red2 = np.array([r4, 255, 255])
    mask_red2 = cv2.inRange(hsv, lower_red2, upper_red2)
    hsv[mask_red2 == 255, 1] = np.clip(hsv[mask_red2 == 255, 1] * s1, 0, 100)
    ...
    # hsv[:, :, 1] = np.where(mask == 255, np.clip(hsv[:, :, 1] * 1.5, 0, 255), hsv[:, :, 1])
...
def aos_response_video():
    ...
    c1 = float(request.args.get('c1', 181))
    c2 = float(request.args.get('c2', 231))
    s1 = float(request.args.get('s1', 1.0)) * 10
    s2 = float(request.args.get('s2', 1.0)) * 10
    print(f'r1: {r1}, r2: {r2}, r3: {r3}, r4: {r4}, c1: {c1}, c2: {c2}, s1: {s1}, s2: {s2}')
